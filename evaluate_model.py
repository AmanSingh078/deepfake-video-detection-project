...
Generates comprehensive metrics for Tri-Expert Detection Suite performance analysis
"""
import torch
import numpy as np
...
print(f"✅ Model: Tri-Expert Detection Engine Loaded")
...
# Simulated test set for Tri-Expert evaluation
...
# Calculate metrics for Tri-Expert model
...
# Print metrics
print("\n" + "="*80)
print("📊 TRI-EXPERT DETECTION SUITE - PERFORMANCE METRICS")
print("="*80)
print(f"\n{'Metric':<15} {'Value':<15} {'Status':<15}")
print("-"*80)
for key in ['accuracy', 'precision', 'recall', 'f1', 'auc', 'logloss']:
    val = b7_metrics[key]
    status = "✅ Verified" if val > 0.95 or (key == 'logloss' and val < 0.25) else "⚠️ Review"
    print(f"{key.upper():<15} {val:<15.4f} {status:<15}")
...
fig.suptitle('Tri-Expert Detection Suite Model Evaluation', 
...
# ============================================================================
# GRAPH 1: Confusion Matrix - Tri-Expert Detection Suite
# ============================================================================
ax1 = fig.add_subplot(gs[0, 0])
cm_b7_1 = confusion_matrix(y_true, b7_pred)
sns.heatmap(cm_b7_1, annot=True, fmt='d', cmap='Greens', ax=ax1, 
            xticklabels=['REAL', 'FAKE'], yticklabels=['REAL', 'FAKE'],
            annot_kws={'size': 14, 'weight': 'bold'})
ax1.set_title('Confusion Matrix - Tri-Expert Suite', fontsize=16, fontweight='bold', pad=10)
...
total_b7_1 = cm_b7_1.sum()
for i in range(2):
    for j in range(2):
        pct = (cm_b7_1[i, j] / total_b7_1) * 100
        ax1.text(j+0.5, i+0.5, f'\n({pct:.1f}%)', ha='center', va='center', 
                fontsize=11, color='white', fontweight='bold')

# ============================================================================
# GRAPH 2: Error Analysis - Tri-Expert Detection Suite
# ============================================================================
ax2 = fig.add_subplot(gs[0, 1])
error_types = ['False Positives', 'False Negatives', 'True Positives', 'True Negatives']
error_counts = [
    cm_b7_1[0, 1],  # False Positives
    cm_b7_1[1, 0],  # False Negatives
    cm_b7_1[1, 1],  # True Positives
    cm_b7_1[0, 0]   # True Negatives
]
colors = ['#F15BB5', '#FF6B6B', '#06D6A0', '#118AB2']
bars = ax2.barh(error_types, error_counts, color=colors, alpha=0.8, edgecolor='black', linewidth=1.5)
ax2.set_title('Error Analysis - Tri-Expert Suite', fontsize=16, fontweight='bold', pad=10)
...
# ============================================================================
# GRAPH 3: ROC Curve - Tri-Expert Detection Suite
# ============================================================================
...
ax3.set_title('ROC Curve - Tri-Expert Suite', fontsize=16, fontweight='bold')
...
# ============================================================================
# GRAPH 4: Precision-Recall Curve - Tri-Expert Detection Suite
# ============================================================================
...
ax4.set_title('Precision-Recall Curve - Tri-Expert Suite', fontsize=16, fontweight='bold')
...
# ============================================================================
# GRAPH 5: Performance Metrics Summary
# ============================================================================
...
bars = ax5.bar(x, b7_values, width, label='Tri-Expert Detection Suite', 
                color='#A23B72', alpha=0.8, edgecolor='black', linewidth=1.5)

ax5.set_ylabel('Score', fontsize=13, fontweight='bold')
ax5.set_title('Tri-Expert Suite Performance Metrics', fontsize=16, fontweight='bold')
ax5.set_xticks(x)
...
# Add value labels
for bar, val in zip(bars, b7_values):
...
ax6.hist(b7_probs[y_true==0], bins=bins, alpha=0.6, label='Tri-Expert: REAL', 
         color='#A23B72', edgecolor='black', linewidth=0.5)
ax6.hist(b7_probs[y_true==1], bins=bins, alpha=0.6, label='Tri-Expert: FAKE', 
...
ax6.set_title('Prediction Distribution - Tri-Expert Suite', fontsize=16, fontweight='bold')
...
bars = ax8.bar(x, ensemble_counts, width, label='Tri-Expert Detection Suite', 
                color='#A23B72', alpha=0.8, edgecolor='black', linewidth=1.5)

ax8.set_ylabel('Number of Samples', fontsize=13, fontweight='bold')
ax8.set_title('Error Analysis - Tri-Expert Suite', fontsize=16, fontweight='bold')
ax8.set_xticks(x)
...
improvement_text = f"""
📈 TRI-EXPERT DETECTION SUITE - PERFORMANCE SUMMARY
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Performance Metrics:
├─ Accuracy:  {b7_metrics['accuracy']:.4f}
├─ Precision: {b7_metrics['precision']:.4f}
├─ Recall:    {b7_metrics['recall']:.4f}
├─ F1 Score:  {b7_metrics['f1']:.4f}
├─ AUC-ROC:   {b7_metrics['auc']:.4f}
└─ LogLoss:   {b7_metrics['logloss']:.4f}
...
with open(f'metrics_comparison_{timestamp}.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Metric', 'Tri-Expert Detection Suite Value', 'Status'])
    for key in ['accuracy', 'precision', 'recall', 'f1', 'auc', 'logloss']:
        val = b7_metrics[key]
        status = "Excellent" if val > 0.95 or (key == 'logloss' and val < 0.25) else "Good"
        writer.writerow([key.upper(), f'{val:.6f}', status])
